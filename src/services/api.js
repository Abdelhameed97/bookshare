...
const apiService = {
    ...
    confirmStripePayment: paymentId =>
        api.post("/stripe/confirm-payment", { payment_id: paymentId }),

    // PayPal Payment
    createPayPalPayment: orderId =>
        api.post("/paypal/create-payment", { order_id: orderId }).catch(err => {
            if (err.response?.status === 400) {
                throw new Error(
                    err.response.data?.message || "Order cannot be paid"
                )
            }
            throw err
        }),
