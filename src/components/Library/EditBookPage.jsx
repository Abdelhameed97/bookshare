...
const EditBookPage = () => {
  ...
  const handleImageChange = (e) => {
    ...
    const validFiles = files.filter(file => {
      ...
      if (!isValidType) Swal.fire('Invalid File', `${file.name} is not a valid image file.`, 'error');
      if (!isValidSize) Swal.fire('File Too Large', `${file.name} is too large. Max 2MB.`, 'error');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await Swal.fire({ icon: 'success', title: 'Book Updated!', text: `${formData.title} has been updated.`, timer: 2000 });
      navigate('/dashboard');
    } catch (error) {
  ...
  if (fetching) return <div style={{ padding: 40, textAlign: 'center' }}>Loading book data...</div>;
