...
const Dashboard = () => {
  ...
  const filteredOrders = useMemo(() => {
    let filtered = allOrders.filter((order) =>
      order.order_items.some((item) =>
        allBooks.some(
          (book) => book.id === item.book_id && book.user_id === currentUser?.id
        )
      )
    );
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      ...
      const [booksResponse, ordersResponse, categoriesResponse] =
      ...
      const userBooks =
        booksResponse.status === "fulfilled" && booksResponse.value.data.data
      ...
      const userOrders =
        ordersResponse.status === "fulfilled" && ordersResponse.value.data.data
      ...
      const totalRevenue = userOrders.reduce((sum, order) => {
        return (
          ...
          order.order_items.reduce((itemSum, item) => {
            ...
            return (
              itemSum + (book ? parseFloat(book.price) * item.quantity : 0)
            );
          }, 0)
        );
      }, 0);

      const uniqueCustomers = new Set(userOrders.map((order) => order.user_id))
        .size;
  ...
  const getBookImageUrl = (book) => {
    let img =
      book.images && book.images.length > 0 ? book.images[0] : book.image;
                  ...
                  <div className="notifications-scroll-container">
                    {pendingOrders.length === 0 ? (
                      ...
                      pendingOrders.slice(0, 2).map((order) => {
                        const userName =
                          order.user?.name || order.client?.name || "Someone";
                        const firstBook =
                          order.order_items?.[0]?.book?.title || "a book";
                        const moreCount =
          ...
          <div className="stat-card">
            ...
            <div className="stat-content">
              ...
              <div className="stat-details">
                ...
                {stats.soldBooks > 0 && (
                  <span className="sold">{stats.soldBooks} sold</span>
                )}
              </div>
            </div>
          </div>
                    ...
                    <div key={book.id} className="book-card">
                      ...
                      <div className="book-info">
                        ...
                        <p className="book-author">
                          By {book.author || book.user?.name || "Unknown"}
                        </p>
