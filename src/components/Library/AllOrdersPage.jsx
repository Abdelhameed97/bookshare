...
const AllOrdersPage = () => {
  ...
  useEffect(() => {
    ...
    if (search) {
      ...
      filtered = filtered.filter(order =>
        ...
        order.order_items?.some(item => item.book?.title?.toLowerCase().includes(s))
      );
    }
  ...
  const handleStatusChange = async (orderId, action) => {
    ...
    if (action === 'accept') {
      apiCall = () => apiService.post(`/orders/${orderId}/accept`);
      confirmText = 'Are you sure you want to accept this order?';
      successText = 'Order accepted!';
    } else if (action === 'reject') {
      apiCall = () => apiService.post(`/orders/${orderId}/reject`);
