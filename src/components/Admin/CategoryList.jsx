...
const CategoryList = () => {
  ...
  const [form, setForm] = useState({ name: '', type: 'general', customType: '' });
  ...
  const handleAddOrEditCategory = async (e) => {
    ...
    if (form.type === 'other') {
      ...
      dataToSend.type = form.customType;
    }
    delete dataToSend.customType;
    try {
      if (editId) {
    ...
    <>
      ...
      <div className="dashboard-container">
        <div className="admin-page-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
          <h1 style={{ fontWeight: 700, color: '#1e293b' }}>Manage Categories</h1>
          <button className="btn-add-new" style={{ background: '#3b82f6', color: 'white', borderRadius: 8, padding: '0.6rem 1.2rem', fontWeight: 600, border: 'none', display: 'flex', alignItems: 'center', gap: 8 }} onClick={() => openModal()}>
          ...
          <table className="admin-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {currentCategories.length === 0 && (
                <tr><td colSpan={4} style={{ textAlign: 'center', color: '#888', padding: '2rem' }}>No categories found.</td></tr>
              )}
              {currentCategories.map((category) => (
                <tr key={category.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
                  ...
                  <td style={{ padding: '0.75rem' }}>{category.type}</td>
                  <td style={{ padding: '0.75rem' }}>
                    <button className="btn-edit" style={{ background: '#10b981', color: 'white', border: 'none', borderRadius: 6, padding: '0.4rem 0.7rem', marginRight: 8 }} onClick={() => openModal(category)}><FaEdit /></button>
                    <button className="btn-delete" style={{ background: '#ef4444', color: 'white', border: 'none', borderRadius: 6, padding: '0.4rem 0.7rem' }} onClick={() => handleDelete(category.id)}><FaTrash /></button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {showModal && (
          <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.18)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: 'white', borderRadius: 12, padding: '2rem', minWidth: 340, boxShadow: '0 4px 24px rgba(0,0,0,0.13)', position: 'relative' }}>
              ...
              <h2 style={{ marginBottom: 20, color: '#1e293b', fontWeight: 700 }}>{editId ? 'Edit Category' : 'Add Category'}</h2>
              <form onSubmit={handleAddOrEditCategory}>
                ...
                <div style={{ marginBottom: 18 }}>
                  <label style={{ display: 'block', marginBottom: 6, color: '#374151', fontWeight: 600 }}>Type</label>
                  <select name="type" value={form.type} onChange={handleFormChange} style={{ width: '100%', padding: '0.6rem', borderRadius: 6, border: '1px solid #d1d5db', fontSize: 16 }}>
                ...
                {form.type === 'other' && (
                  <div style={{ marginBottom: 18 }}>
                    <label style={{ display: 'block', marginBottom: 6, color: '#374151', fontWeight: 600 }}>Custom Type</label>
                    <input type="text" name="customType" value={form.customType} onChange={handleFormChange} style={{ width: '100%', padding: '0.6rem', borderRadius: 6, border: '1px solid #d1d5db', fontSize: 16 }} placeholder="Enter custom type..." />
                  </div>
                )}
                {formError && <div style={{ color: '#ef4444', marginBottom: 10 }}>{formError}</div>}
                <button type="submit" disabled={adding} style={{ background: '#3b82f6', color: 'white', border: 'none', borderRadius: 8, padding: '0.7rem 1.5rem', fontWeight: 600, fontSize: 16, width: '100%' }}>
                  {adding ? (editId ? 'Saving...' : 'Adding...') : (editId ? 'Save Changes' : 'Add Category')}
                </button>
              </form>
            </div>
          </div>
