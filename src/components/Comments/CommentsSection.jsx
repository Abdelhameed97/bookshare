...
const CommentsSection = ({ bookId, currentUser }) => {
  ...
  const renderComment = (comment, isReply = false) => {
    ...
    return (
      <div
        key={comment.id}
        className={`border rounded-lg p-4 mb-4 ${isReply ? "ml-6 bg-gray-50" : "bg-white"}`}
      >
        ...
        {editingCommentId === comment.id ? (
          ...
          <p className="text-gray-800 mt-1 whitespace-pre-line">{comment.comment}</p>
        )}
      ...
      {currentUser ? (
        <form onSubmit={handleCommentSubmit} className="mb-6">
          <textarea
            ...
            rows={3}
            placeholder={
              replyTo ? "Write a reply..." : "Write a comment about this book..."
            }
            value={newComment}
